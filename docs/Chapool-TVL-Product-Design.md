...
- **建议路线**: `Chapool Earn (USDT Vault)` → `veCPOT 锁仓` → `NFT Boost` → `NFT 抵押借贷`
...
- `CPP`: 平台内部流通与激励代币，负责奖励发放、生态消费，以及提现到 `U卡` 后进行线下消费

说明：

- 当前仓库里的链上实现仍以 `CPPToken` 命名为主
- 本文档讨论的是后续产品化方向，因此采用 `CPOT + CPP` 的业务视角
...
建议 Chapool 按下面四层结构推进：

| 模块 | 角色 | 是否建议首期上线 | 对 TVL 的贡献 |
|------|------|------------------|---------------|
| `Chapool Earn` | USDT 理财金库 | 是 | 核心 TVL 来源 |
| `veCPOT` | CPOT 锁仓治理与权益 | 是 | 辅助 TVL 与留存 |
| `NFT Boost` | 持 NFT 提升收益 | 是 | 提升 NFT 生态价值 |
| `NFT Loan` | NFT 抵押借贷 | 否，二期 | 放大 TVL 与资金效率 |

### 为什么先做 `Chapool Earn`

这是最符合你们现状、最容易形成有效 TVL 的产品，原因如下：

1. 用户对 `USDT 存入赚收益` 的理解成本最低
2. TVL 可以直接定义为 `Vault 合约内的 USDT 余额`
3. 可以最快接入 DefiLlama
4. 可以自然结合 `CPOT`、`CPP` 与 `CPNFT`
5. 相比 P2P 借贷，合约复杂度和风控复杂度更低
...
| 产品 | 描述 | 流动性 | 风险 | 建议 |
|------|------|--------|------|------|
| `活期 Earn` | 随存随取，收益浮动 | 高 | 低 | 首发必做 |
| `定期 Earn` | 7/30/90 天锁定，收益更高 | 中 | 中 | 首发可选 |
...
`veCPOT` 是 CPOT 的锁仓权益模块，不建议把它单独作为 TVL 主产品，而应作为 `Earn` 的权益引擎与平台会员中枢。
...
锁仓奖励发放的 `CPP` 不建议只做提现型收益，而应同时承担生态消费功能：
...
二期可以把 `Earn Vault` 的部分资金作为借贷流动性来源，但建议在首期先逻辑隔离：
...
CPP 激励:
按活动规则发放 CPP，提升吸引力，并进入站内消费或 U卡线下消费路径
```
...
而当 `CPP` 可以提现到 `U卡` 并用于线下消费时，它就有了明确的使用出口：
...
若后续 `CPOT` 已公开流通、可定价且锁在可验证合约中，可以单独作为“锁仓价值”或扩展 TVL 口径进行展示。  
但首期建议对外主口径仍然保持简单：
...
| `CPPUtilityRouter` | CPP 消费、提现到 U卡、线下消费映射接口 |
...
P2P 借贷可以做，但不适合作为 TVL 冷启动产品。  
如果目标是先建立协议资产沉淀和外部排名，`Earn` 明显更优。
...
- 不建议直接复用 `Payment` 作为 TVL 池
- 可以复用其 ERC20 收付经验和白名单思路
