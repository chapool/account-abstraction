...
describe("AAWallet - Proxy Pattern Test", function () {
    ...
    it("Should not allow initialization on implementation contract", async function () {
        ...
        await expect(walletImplementation.initialize(ownerAddress, ethers.constants.AddressZero))
            .to.be.revertedWithCustomError(walletImplementation, "InvalidInitialization");
    ...
    it("Should deploy and initialize via proxy", async function () {
        ...
        try {
            ...
            // Alternative: Test using WalletManager which should handle proxy deployment
            ...
            await walletManager.initialize(
                entryPoint.address,
                ethers.constants.AddressZero, // cpopToken address - not needed for this test
                ownerAddress
            );
    ...
    it("Should test basic wallet functionality after proper initialization", async function () {
        ...
        // Use WalletManager to create a properly initialized wallet
        const WalletManagerFactory = await ethers.getContractFactory("WalletManager");
        const walletManager = await WalletManagerFactory.deploy();
        await walletManager.deployed();
        ...
        const salt = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("functional_test"));
        await walletManager.createAccount(ownerAddress, salt);
